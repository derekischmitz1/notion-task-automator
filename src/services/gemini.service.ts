...
export async function parseTaskFromEmail(emailBody: string, maxRetriesPerModel = 2): Promise<ExtractedTask[]> {
  const prompt = [
    ...
    '   - ALWAYS include a task named "1. GET DA UPPY" under the category "1. General", even if it is not present in the email text.',
    '   - ALL tasks assigned to "1. General" MUST be sequentially numbered in their "taskName", preserving the order they appeared in the email (e.g., "1. GET DA UPPY", "2. Put on watch and ring", "3. Fill up Mtn Dew").',
    '',
    ...
    '   - Lines following a timed event that DO NOT have their own timestamp (e.g., "Brush teeth", "Put on deodorant", "Get dressed", "Shave", "Put hair up", "Play laser with Duster") MUST be assigned to "1. General" and continue the sequential numbering.',
    '',
    ...
    '[',
    '  { "taskName": "1. GET DA UPPY", "category": "1. General" },',
    '  { "taskName": "2. Brush teeth", "category": "1. General" },',
