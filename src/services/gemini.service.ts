...
export async function parseTaskFromEmail(emailBody: string, maxRetriesPerModel = 2): Promise<ExtractedTask[]> {
  const prompt = [
    ...
    '   - Convert the time into 24-hour HHMM format followed by a colon and the task name, and prefix it with the category number ("2."):',
    ...
    '   - Lines following a timed event that DO NOT have their own timestamp (e.g., "Brush teeth", "Put on deodorant", "Get dressed", "Shave", "Put hair up", "Play laser with Duster") MUST be assigned to "1. General" and continue the sequential numbering (e.g., "1.4. Brush teeth").',
    '',
    ...
    '',
    '6. **Preserve Priority Numbers and Prefix with Category Number**:',
    '   - When extracting numbered academic items (e.g., "9. PUH 201-F..."), PRESERVE the item number and prefix it with the assigned category number.',
    ...
    '  { "taskName": "1.2. Brush teeth", "category": "1. General" },',
